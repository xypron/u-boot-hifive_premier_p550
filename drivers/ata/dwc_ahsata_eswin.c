...
#define HSP_CSR_BASE_ADDR             0x50440000
...
int eswin_ahci_write(void *addr, unsigned int val)
{
    /* 32 bit write */
				  ...
				  struct blk_desc *pdev)
{
	...
	flush_cache((ulong)(id), ATA_ID_WORDS);
...
{
	u32 rc;
	flush_cache((ulong)(buffer), ATA_SECT_SIZE * blkcnt);
	if (desc->lba48)
...
static int eswin_sata_clk_enable(struct udevice *dev)
{
    int ret = 0;
	struct clk *cfg_clk;
	cfg_clk = devm_clk_get(dev, "hsp_cfg_clk");
	if (IS_ERR_OR_NULL(cfg_clk)) {
...
static int eswin_sata_init_cfg(void)
{
    ...
    eswin_ahci_write((void *)(hsp_io_mem + SATA_PHY_CTRL0), (SATA_P0_AMPLITUDE_GEN1|SATA_P0_AMPLITUDE_GEN2|SATA_P0_AMPLITUDE_GEN3));
    eswin_ahci_write((void *)(hsp_io_mem + SATA_PHY_CTRL1), (SATA_P0_PHY_TX_PREEMPH_GEN1|SATA_P0_PHY_TX_PREEMPH_GEN2|SATA_P0_PHY_TX_PREEMPH_GEN3));
    eswin_ahci_write((void *)(hsp_io_mem + SATA_LOS_IDEN), SATA_LOS_LEVEL|SATA_LOS_BIAS);
    eswin_ahci_write((void *)(hsp_io_mem + SATA_AXI_LP_CTRL), (SATA_M_CSYSREQ|SATA_S_CSYSREQ));
    eswin_ahci_write((void *)(hsp_io_mem + SATA_REG_CTRL), (SATA_REF_REPEATCLK_EN|SATA_REF_USE_PAD));
    eswin_ahci_write((void *)(hsp_io_mem + SATA_MPLL_CTRL), SATA_MPLL_MULTIPLIER);
    eswin_ahci_write((void *)(hsp_io_mem + SATA_RESET_CTRL), 0x0);
	return 0;
}
...
int dwc_ahsata_probe(struct udevice *dev)
{
	...
	ret = eswin_sata_clk_enable(dev);
    if (ret) {
        return ret;
    }
    ret = eswin_sata_reset(dev);
    if (ret) {
        return ret;
    }
	eswin_sata_init_cfg();
