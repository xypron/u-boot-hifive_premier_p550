...
static const struct eswin_pin_desc eswin_pins[] = {
...
#define ESWIN_PINMUX_REG_OFFSET   (0x04)
...
/* pinmux */
static int eswin_pmux_get_pins_count(struct udevice *dev)
{
	return ARRAY_SIZE(eswin_pins);
};

static const char *eswin_pmux_get_pin_name(struct udevice *dev, unsigned selector)
{
	if (selector < ARRAY_SIZE(eswin_pins))
		return eswin_pins[selector].name;
	return NULL;
}

static int eswin_get_pin_muxing(struct udevice *dev, unsigned int selector,
			      char *buf, int size)
{
	int reg ;
	struct eswin_pinctrl *priv = dev_get_priv(dev);

	reg = readl(priv->base + selector * ESWIN_PINMUX_REG_OFFSET);
	reg &= ESWIN_PINMUX_MASK;
	reg >>=  ESWIN_PINMUX_SHIFT;

	snprintf(buf, size, "Func.%d", reg);
	return 0;
}

static int eswin_pctrl_get_groups_count(struct udevice *dev)
{
	struct eswin_pinctrl *priv = dev_get_priv(dev);
	return priv->ngroups;
}

static const char *eswin_pctrl_get_group_name(struct udevice *dev,
...
static int eswin_pmux_get_functions_count(struct udevice *dev)
{
	struct eswin_pinctrl *priv = dev_get_priv(dev);
...
static const char *eswin_pmux_get_function_name(struct udevice *dev,
					 unsigned int selector)
{
	struct eswin_pinctrl *priv = dev_get_priv(dev);
...
static int eswin_pinmux_set(struct udevice *dev, unsigned int pin_selector,
			  unsigned int func_selector)
{
	struct eswin_pinctrl *priv = dev_get_priv(dev);
	const struct eswin_function_desc *func = &priv->funcs[func_selector];
	u32 reg ;

	reg = readl(priv->base + pin_selector * ESWIN_PINMUX_REG_OFFSET);
	reg &= ~ESWIN_PINMUX_MASK;
	reg |= (func->mux_val << ESWIN_PINMUX_SHIFT);
	writel(reg, priv->base + pin_selector * ESWIN_PINMUX_REG_OFFSET);

	return 0;
}

static int eswin_pinmux_group_set(struct udevice *dev,
				unsigned int group_selector,
				unsigned int func_selector)
{
	int i ;
	struct eswin_pinctrl *priv = dev_get_priv(dev);
	const struct eswin_group_desc *pgrp = &priv->groups[group_selector];
	const struct eswin_function_desc *func = &priv->funcs[func_selector];

	for(i = 0 ;i< pgrp->npins;i++){
		u32 reg ;
		unsigned int pin = pgrp->pins[i];
		reg = readl(priv->base + pin * ESWIN_PINMUX_REG_OFFSET);
		reg &= ~ESWIN_PINMUX_MASK;
		reg |= (func->mux_val << ESWIN_PINMUX_SHIFT);
		writel(reg, priv->base + pin * ESWIN_PINMUX_REG_OFFSET);
	}
	return 0;
}
...
static const struct pinconf_param eswin_pinconf_params[] = {
...
static int eswin_pinconf_cfg_set(struct udevice *dev, unsigned int pin,
...
static int eswin_pinconf_group_set(struct udevice *dev,
				 unsigned int group_selector,
				 unsigned int param, unsigned int arg)
{
	struct eswin_pinctrl *priv = dev_get_priv(dev);
	const struct eswin_group_desc *grp =
			&priv->groups[group_selector];
	int i, ret;
	for (i = 0; i < grp->npins; i++) {
		ret = eswin_pinconf_cfg_set(dev, grp->pins[i], param, arg);
		if (ret)
			return ret;
	}
...
const struct pinctrl_ops eswin_pinctrl_ops = {
	.get_pins_count = eswin_pmux_get_pins_count,
	.get_pin_name = eswin_pmux_get_pin_name,
	.get_pin_muxing = eswin_get_pin_muxing,
	...
	.pinmux_set = eswin_pinmux_set,
	.pinmux_group_set = eswin_pinmux_group_set,
#if CONFIG_IS_ENABLED(PINCONF)
	.pinconf_num_params = ARRAY_SIZE(eswin_pinconf_params),
	.pinconf_params = eswin_pinconf_params,
	.pinconf_set = eswin_pinconf_cfg_set,
	.pinconf_group_set = eswin_pinconf_group_set,
#endif
...
static int eswin_pinctrl_probe(struct udevice *dev)
{
	struct eswin_pinctrl *priv = dev_get_priv(dev);

	priv->groups = eswin_pinctrl_groups;
	priv->ngroups = ARRAY_SIZE(eswin_pinctrl_groups);
	priv->funcs = eswin_pinmux_functions;
	priv->nfuncs = ARRAY_SIZE(eswin_pinmux_functions);

	priv->base = dev_read_addr_ptr(dev);
	if (!priv->base)
		return -EINVAL;
	
	dev_info(dev, ": base %p,  %d groups, %d funcs\n",
		 priv->base, priv->ngroups, priv->nfuncs);
	return 0;
}
...
U_BOOT_DRIVER(eswin_pinctrl) = {
	.name = "eswin_pinctrl",
	.id = UCLASS_PINCTRL,
	.of_match = eswin_pinctrl_of_match,
	.ops = &eswin_pinctrl_ops,
	.probe = eswin_pinctrl_probe,
	.priv_auto = sizeof(struct eswin_pinctrl),
};
